-- Active: 1713241729476@@127.0.0.1@3306@tugas_akhir
...
CREATE TABLE IF NOT EXISTS repository_jti.view (
  ...
  id_user VARCHAR(15) NOT NULL,
  id_document VARCHAR(10) NOT NULL,
  INDEX fk_view_user1_idx (id_user ASC),
...
CREATE TABLE IF NOT EXISTS repository_jti.tracking_doc (
  id_tracking INT NOT NULL AUTO_INCREMENT,
  status ENUM('Success', 'Pending') NULL,
  description VARCHAR(150) NULL,
  id_user VARCHAR(15) NOT NULL,
  id_document VARCHAR(10) NOT NULL,
  PRIMARY KEY (id_tracking),
  INDEX fk_tracking_doc_user1_idx (id_user ASC),
  INDEX fk_tracking_doc_document1_idx (id_document ASC),
  CONSTRAINT fk_tracking_doc_user1
    FOREIGN KEY (id_user)
    REFERENCES repository_jti.user (id_user)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_tracking_doc_document1
    FOREIGN KEY (id_document)
    REFERENCES repository_jti.document (id_document)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS repository_jti.notification (
  id_user VARCHAR(15) NOT NULL,
  id_document VARCHAR(10) NOT NULL,
  INDEX fk_notification_user1_idx (id_user ASC),
